...
const Dashboard: React.FC = () => {
  const { user, updateUserProgress, removeUser } = useUser();
  ...
  const handleDeleteProfile = useCallback(() => {
    if (user && confirm('Are you sure you want to delete this profile? This action cannot be undone.')) {
      removeUser(user.name);
      router.push('/');
    }
  }, [user, removeUser, router]);
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="mb-8">
        <div>
          ...
          <p>You&apos;ve mastered {wordsMastered} words, with an accuracy of {accuracyRate.toFixed(0)}% at {averageTimePerWord.toFixed(1)}s per word.</p>
