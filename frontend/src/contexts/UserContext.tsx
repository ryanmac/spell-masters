...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const toggleTypingMode = useCallback(() => {
    setUsers(prevUsers => {
      if (user) {
        const updatedUser = { ...user, typingMode: !user.typingMode };
        return { ...prevUsers, [user.name]: updatedUser };
      }
      return prevUsers;
    });
  ...
  return (
    <UserContext.Provider value={{ user, setUser, updateUserProgress, updateLevelProgress, userListVersion, removeUser, getAllUsers, logout, toggleTypingMode }}>
      {children}
